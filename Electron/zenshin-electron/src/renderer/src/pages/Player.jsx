...
export default function Player(query) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:8000/add/${encodeURIComponent(magnetURI)}`)
      console.log(response)
      // Step 2: Set the video source for streaming
      setVideoSrc(`http://localhost:8000/stream/${encodeURIComponent(magnetURI)}`)
      // Step 3: Set the subtitle source
      setSubtitleSrc(`http://localhost:8000/subtitles/${encodeURIComponent(magnetURI)}`)
    } catch (error) {
  ...
  const handleVlcStream = async () => {
    try {
      await axios.get(`http://localhost:8000/add/${encodeURIComponent(magnetURI)}`)
  ...
  const handleRemoveTorrent = async () => {
    try {
      // Send a DELETE request to remove the torrent
      await axios.delete(`http://localhost:8000/remove/${encodeURIComponent(magnetURI)}`)
