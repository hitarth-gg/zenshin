...
function extractData(data) {
  ...
  const coverMatch = data.match(/<a href="(https:\/\/i.animepahe.pw\/posters.+?)"/)
...
router.get('/details', cookieMiddleware, async (req, res) => {
  try {
    ...
    // const cover = data.match(/<a href="(https:\/\/i.animepahe.pw\/posters.+?)"/)[1]
...
// https://i.animepahe.pw/snapshots/069b876a55ac41fbbe3fc992f04297d7902b204a257762ed15a4c2901c21b28f.jpg
...
router.get('/image/poster/:id', cookieMiddleware, async (req, res) => {
  ...
  try {
    ...
    const urls = [
      `https://i.animepahe.pw/posters/${id}`,
      `https://i.animepahe.pw/uploads/posters/${id}`
    ]
