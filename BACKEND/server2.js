...
/* ------------------------------------------------------ */
/* ------------------------------------------------------ */
let subtitleStream = new SubtitleStream(); // To hold the current SubtitleStream instance
let currentMediaFile = ""; // To hold the current media file being streamed
const subtitles = [];
...
app.get("/streamfile/:magnet/:filename", async function (req, res, next) {
  ...
  const readStream1 = new ReadableStreamClone(stream);
  const readStream2 = new ReadableStreamClone(stream);
  readStream1.pipe(res);


  subtitleStream.once("tracks", (tracks) => {
    subtitleStream.on("subtitle", (subtitle, trackNumber) => {
      console.log(`Subtitle from Track ${trackNumber}:`, subtitle);
      // write the subtitle to a file in the current directory
      // fs.writeFileSync("subtitle.txt", subtitle.data, "utf-8");
      // fs.writeFileSync("subtitle_track.txt", "trackNumber", "utf-8");

      subtitles.push({ trackNumber, subtitle });

    });

    console.log("Subtitle tracks:", tracks);
    // fs.writeFileSync("subtitle_tracks.json", JSON.stringify(tracks), "utf-8");
  ...
  if (subtitleStream) {
    readStream2.pipe(subtitleStream);
  }

  stream.on("error", function (err) {
    console.error("Stream error:", err);
    // Only send a response if headers haven't been sent yet
    fs.writeFileSync("error.txt", err, "utf-8");
    if (!res.headersSent) {
...
app.get("/subtitles", (req, res) => {
  // if (!subtitleStream) {
  //   return res.status(404).send("No active subtitle stream found");
  // }
  // const subtitleData = [];
